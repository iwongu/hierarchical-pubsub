...
Node.prototype.setValue = function(value) {
  var event = this.path();
  this.value = value;
  this.walkUp(function(node) {
    node.emitter.emit('value', {event: event, value: value});
  });
};

Node.prototype.walkUp = function(callback) {
  if (this.isRoot()) {
    return;
  }
  callback(this);
  this.parent.walkUp(callback);
};

Node.prototype.walkDown = function(callback) {
  callback(this);
  for (var i = 0; i < this.children.length; i++) {
    var child = this.children[i];
    child.walkDown(callback);
  };
};
...
PS.prototype.get = function(path) {
